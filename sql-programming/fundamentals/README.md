...
같습니다 — 데이터도, **정렬 규칙도**, **세션 설정**(시간대·메시지 언어·날짜
표기·통화/숫자/날짜 이름의 로케일)**도**. 다만 화면이 world가 아니라 **여러분
서버 자신의 사실**(설치 방법·역할 이름 같은 것)을 비추는 자리는 대안 경로에서
다르게 보입니다 (아래 「대안 경로의 알려진 차이」).
...
| `./setup.sh` | 컨테이너 생성·기동(`LANG=C.UTF-8`) → DB 생성 → **세션 설정 고정** → 적재 → 검증 | 컨테이너 없음. psql·서버 접속·버전 18·`bookstore` DB를 점검(없으면 **정렬 규칙과 문자 분류를 고정해** 생성) → **세션 설정 고정** → 적재 → 검증 |
| 경로 지정 | 기본값이므로 지정하지 않아도 됩니다 | **`KIT_MODE=native ./setup.sh` 한 번**. 그 실행이 경로를 `.kit-mode`에 기억하므로 이후 명령에는 붙이지 않아도 됩니다 (아래 「구축 경로 기억」) |
| 정렬 규칙(collation)·문자 분류 | 컨테이너 initdb를 `LANG=C.UTF-8`로 못 박아 얻습니다 | `CREATE DATABASE … TEMPLATE template0 ENCODING 'UTF8' LOCALE_PROVIDER builtin BUILTIN_LOCALE 'C.UTF-8' LC_COLLATE 'C' LC_CTYPE 'C'`로 못 박습니다. **이미 만들어 두신 `bookstore`가 다른 문자 분류(`LC_CTYPE`)로 되어 있어도 그대로 씁니다** — `알림: ` 한 덩어리만 나오고 구축도 입장 점검도 막히지 않습니다 (아래 「문자 분류 알림」) |
| 세션 설정 | `ALTER DATABASE`로 못 박습니다 (아래 「세션 설정 고정」) | 동일 — 같은 `ALTER DATABASE` |
| `./reset.sh` `./check_env.sh` | 동일 | 동일 |
| `./verify.sh` | `cases/`의 케이스 전량 통과 | **여러분 서버 자신의 사실을 비추는 케이스 몇 건이 다릅니다** — 나머지는 전량 PASS입니다 (아래 「대안 경로의 알려진 차이」에 구성별 건수) |
...
kit이 판정하지 않습니다. **그래서 `./check_env.sh`는 통과하는데 psql 화면만
교재와 다르다면 `~/.psqlrc`를 먼저 보세요** — 그 파일을 잠시 다른 이름으로
옮겨 두고(`mv ~/.psqlrc ~/.psqlrc.off`) 다시 접속해 화면이 교재와 같아지면
원인은 그 파일입니다. 기본 경로는 컨테이너 안의 psql이 여러분 컴퓨터의
`~/.psqlrc`를 읽지 않으므로 해당하지 않습니다.
...
같은 데이터와 같은 정렬 규칙 위에서도, 서버가 자기 컴퓨터에서 물려받은
설정이 다르면 화면이 교재와 달라집니다. 특히 **오류 메시지의 언어**가
그렇습니다 — 한국어 로케일로 만든 서버는 `ERROR:  division by zero` 대신
`오류:  0으로는 나눌수 없습니다.`를 냅니다. 이 코스는 오류 메시지를 그대로
싣는 자리가 많으므로, `./setup.sh`가 두 경로 모두에서 `bookstore`
데이터베이스의 기본 설정을 다음과 같이 못 박습니다.

| 설정 | 고정값 | 이것이 갈리면 |
|---|---|---|
| `lc_messages` | `C` | 오류·안내 메시지의 언어가 달라집니다 |
| `timezone` | `UTC` | 지금 시각을 다루는 자리의 표시가 달라집니다 |
| `DateStyle` | `ISO, MDY` | `'01/02/2026'`처럼 모호한 날짜 **입력**의 해석이 달라집니다 |
| `lc_monetary`·`lc_numeric`·`lc_time` | `C` | `to_char`의 통화 기호·자릿수 구분·요일 이름이 달라집니다 |

`./setup.sh`가 성공하면 화면에 `세션 설정 고정: …` 한 줄이 나옵니다.
...
역할 설정입니다(그것이 전부라고 단정하지는 않습니다). 그런 자리를 쓰고 계시다면
`./check_env.sh`가 불일치를 알리므로, 그 자리를 지우거나 값을 위 표에 맞추시면
됩니다.

#### 문자 분류 알림

정렬 규칙과 달리 **문자 분류(`LC_CTYPE`)는 데이터베이스를 만들 때만 정할 수
있습니다.** `./setup.sh`가 `bookstore`를 **새로 만들 때** 그 값이 정해집니다 — 기본
경로에서는 컨테이너가 `C.UTF-8`로 초기화되므로 `C.UTF-8`이 되고, 대안 경로에서는
`./setup.sh`가 `C`로 못 박습니다. 둘 다 코스가 기대하는 C 계열입니다. 다만 이미
만들어 두신 데이터베이스가 다른 값이면 나중에 바꿀 수 없는데, 이 경우 구축과 입장
점검은 **막지 않고 알림만** 냅니다.

```
알림: 데이터베이스 bookstore 의 문자 분류(LC_CTYPE)가 'ko_KR.UTF-8' 입니다 — ./setup.sh 가 새로 만드는 데이터베이스는 C 계열 — C, POSIX, C.UTF-8(C.utf8) 입니다.
```

(이 줄 아래로 여덟 칸 들여쓴 설명이 두 줄 더 나옵니다 — 무엇이 갈리는지와, 굳이
맞추고 싶을 때의 방법입니다.)

이 설정은 **값을 글자 단위로 어떻게 읽을지**를 정합니다. 여기에 딸리는 자리는
넓습니다 — `upper`·`lower` 같은 대소문자 변환, 글자가 어느 종류인지 따지는 판정
(정규식의 문자 클래스, 대소문자를 무시하는 비교), 행을 통째로 하나의 값으로 찍는
출력에서 값에 따옴표가 붙는지(`(김유나)`가 `("김유나")`로 나오는 식), 그리고 1장
1.2절 `\l`이 내는 `Ctype` 열의 값 같은 것들입니다.

다만 **이 코스의 예제와 문제에서 실제로 달라지는 것은 마지막 하나뿐입니다.** 이
코스는 정규식도, 대소문자를 무시하는 비교도 한 번도 쓰지 않습니다. 대소문자 변환은
쓰지만 인자가 모두 ASCII(영문·숫자와 `@`·`.` 같은 기호)라 어느 설정에서나 값이 같고,
행을 통째로 하나의 값으로 찍는 질의도 이 코스에는 없습니다. 마지막 하나인 `\l`은 원래 여러분 서버 자신을
비추는 자리라 아래 「대안 경로의 알려진 차이」가 이미 다룹니다. 그대로 쓰셔도 됩니다.

그래도 맞추고 싶으시면 `dropdb bookstore` 뒤 `./setup.sh`를 다시 실행하세요 —
world는 `seed.sql`에서 다시 적재되므로 잃는 것이 없습니다. 기본 경로의 컨테이너는
언제나 `C.UTF-8`로 초기화되므로 이 알림이 나오지 않습니다.
...
지우라고 안내합니다. 이 파일은 **여러분 컴퓨터의 상태 파일이므로** 받으신 kit
폴더에는 들어 있지 않습니다 — `./setup.sh`가 구축에 성공할 때 만듭니다.
...
값은 교재에 실린 것과 같아질 수 없고, **다르게 나오는 것이 잘못이 아닙니다.**
...
- **객체의 소유자 이름** — `\dt`·`\dt+`(1장)와 `\l`에는 `Owner` 열이 있고,
  거기에는 **kit가 접속한 역할 이름**이 그대로 나옵니다. 교재는 기본 경로의
  `postgres`로 적혀 있지만, 0장 0.7절이 macOS에 권하는 Postgres.app이나
  Homebrew 설치는 `postgres` 역할을 만들지 않고 **여러분 계정 이름**을 관리자로
  둡니다. 그러면 `Owner` 열의 값이 달라지고, 그 이름의 길이에 따라 **열 너비도**
  달라집니다 — `postgres`와 글자 수가 같은 이름(여덟 자)이면 너비는 그대로이고
  값만 갈립니다.

교재에 실린 화면(기본 경로, 역할 `postgres`)의 `\dt` 앞 네 줄입니다.
...
| 관리자 역할 이름이 **여러분의 계정 이름** (`PGUSER`를 비워 두는 구성) | **4건 FAIL**, 나머지 전량 PASS | 위 둘 + `ch01-01-dt`·`ch01-10-dt-plus` (`Owner` 열) |

둘 다 PostgreSQL 18 서버를 직접 띄워 잰 것입니다(2026-09-18 실측 — 서버 로케일
`ko_KR.UTF-8`, 시간대 `Asia/Seoul`). 아래 행은 관리자 이름을 운영체제 계정으로
두는 설치(Postgres.app·Homebrew가 그렇습니다)에서 만나는 구성인데, 실측은 그
**성질을 같게 만든** 서버에서 한 것이고 Postgres.app 자체로 잰 것은 아닙니다.
서버 로케일이 또 다르면 `\l`의 `Collate`·`Ctype` 열이 다시 달라집니다.
...
메시지를 그대로 냅니다. 세 번째 열이 `./check_env.sh`만인 행은 그 스크립트만
보는 검사이므로 다른 두 스크립트에는 나오지 않습니다.
