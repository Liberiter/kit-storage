...
0장의 두 경로(0.1절 기본 경로, 0.7절 대안 경로)를 **같은 스크립트**가 받습니다. 그래서
어느 경로를 쓰셔도 **명령은 그대로**입니다 — 어느 경로로 도는지는 스크립트가 알아서
가릅니다(아래 「구축 경로 기억」). 경로에 따라 무엇이 갈리는지는 아래 표에 있습니다.
...
2. **네 문항 채점** — `entry/q1.sql` ~ `entry/q4.sql`의 머리 주석에 문항이 있습니다. 그
   ...
   `./setup.sh`를 다시 실행해도, `./reset.sh`로 world를 되돌려도 덮이지 않으므로**
   적어 두신 답은 그대로 남습니다. 여러분이 적는 파일이라 받으신 kit의 갱신 대상에서도
   빠져 있습니다(`.gitignore`).
...
kit 스크립트는 **한 번에 하나씩**, 앞의 것이 끝난 뒤에 실행하세요. 같은 world를 쓰는 실행이
겹치면 한쪽이 다른 쪽이 만든 상태를 지워 결과가 그때그때 달라집니다. 그래서 world를 쓰는
동안 「지금 이 world를 쓰는 중」이라는 표시를 남기는 스크립트가 있고, 그 표시를 보고 거절하는
스크립트가 있습니다 — 이 둘은 같지 않습니다.
...
— 기본 경로에서 전량 PASS면 여러분의 world가 교재의 world와 같다는 뜻입니다. 대안 경로에서는
아래 「대안 경로의 알려진 차이」에 적은 케이스가 FAIL로 나오는 것이 정상입니다. 변경형
케이스는 실행 전후에 world를 자동으로 되돌립니다.
...
마지막 줄의 수는 `cases/`에 들어 있는 케이스의 수입니다 — 장이 늘 때마다 그 장의 케이스가
더해지므로 이 수는 커집니다. 위 화면은 2026-09-12에 기본 경로에서 받은 것입니다.

### 대안 경로의 알려진 차이

갈리는 것은 예제 데이터가 아니라 **여러분 서버 자신의 사실**입니다 — 그 서버를 어떤 방법으로
설치했는지, 어떤 설정으로 돌고 있는지 같은 것들입니다(0장 0.7절이 같은 이야기를 합니다).
예제 데이터(world)는 두 경로에서 같습니다. 그래서 교재가 **여러분 서버 자신의 사실**을 글자
그대로 고정해 둔 케이스는 대안 경로에서 어긋난 것으로 나오고, **그것이 정상입니다** —
여러분이 잘못 설치한 것도, world가 다른 것도 아닙니다.

지금까지 확인된 자리는 다음과 같습니다. 새로 알게 되면 여기에 보태겠습니다 — 장이 늘 때마다
그 장의 케이스가 더해지므로 이런 자리도 늘 수 있습니다.

- **`ch00-01-show-server-version`** — 0장 0.5절에서 psql로 접속한 뒤 입력하는
  `SHOW server_version;`의 출력입니다. 이 값에는 버전 번호 뒤에 서버를 만든 방법의 표시가
  붙습니다 — 기본 경로의 컨테이너는 `18.6 (Debian 18.6-1.pgdg13+2)`, Homebrew로 설치한
  서버는 `18.6 (Homebrew)`입니다(열 너비까지 달라집니다). 그래서 대안 경로에서
...
Asia/Seoul·로케일 en_US.UTF-8로 초기화한 것)에서도 컨테이너와 같은 결과(8 PASS)를 냅니다.
그렇게 되는 조건이 `./setup.sh`가 고정하는 **세션 시간대(UTC)와 메시지 언어**입니다 —
고정하지 않으면 `timestamptz`가 `+09`로 표시되고 오류 메시지가 바뀌어
