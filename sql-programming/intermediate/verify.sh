...
#   (변경형 케이스 — DDL·DML·트랜잭션·UPSERT·트리거·마지막 평가 장). 실행 후에도 남은
...
for sql in "${files[@]}"; do
  ...
  if [ $is_reset = 1 ]; then
    ./reset.sh >/dev/null || { echo "오류: 케이스 $name 실행 전 world 초기화에 실패했습니다 (위 reset 메시지 참고)." >&2; exit 2; }
  fi

  actual=$(run_case "$sql")

  if [ $is_reset = 1 ]; then
    ./reset.sh >/dev/null || { echo "오류: 케이스 $name 실행 후 world 초기화에 실패했습니다 (위 reset 메시지 참고)." >&2; exit 2; }
  fi
...
if [ $failed -gt 0 ]; then
  ...
  echo "  다음: 먼저 다른 ./verify.sh·./reset.sh·./entry_check.sh 가 같은 world를 쓰고 있지 않았는지 확인하세요(겹치면 결과가 흔들립니다). 아니라면 diff 의 차이를 본문·케이스와 대조하세요 — world가 의심되면 ./reset.sh 뒤 다시 실행."
  exit 1
fi
