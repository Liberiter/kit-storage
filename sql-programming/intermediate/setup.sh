...
# 정할 수 있고, 이 코스의 출력에서 이 축에 걸리는 자리는 지금까지 0건이라(kit_psql.sh
# 「libc 문자 분류」 — 서버 자신의 사실을 비추는 \l 과 행 전체를 한 값으로 찍는 출력을 이
# 코스가 쓰지 않는다), 다른 값으로 이미 만들어져 있는 데이터베이스는 막지 않고 알림만 낸다.
#
...
if [ "$KIT_MODE" = native ]; then
  ...
  # 이미 있던 $DB의 libc 문자 분류(LC_CTYPE)가 C 계열이 아니어도 **막지 않는다.** 이 축이
  # 영향을 주는 자리는 넓지만(확인된 목록은 kit_psql.sh 「libc 문자 분류」 — 전부라고 단정하지
  # 않는다), 그 가운데 LC_CTYPE «에만» 매인 것은 행 전체를 한 값으로 찍는 출력의 인용과 \l 의
  # Ctype 열이고, 이 코스의 예제·문제는 그 자리를 지금까지 한 번도 쓰지 않는다. 대소문자 변환·
  # 정규식·무시 비교는 제공자와 그 로케일이 맡으므로 datctype 으로는 판정되지도 않는다.
